...
KNOWLEDGE_GRAPH_PROMPT_TEMPLATE = """你是一位教学知识工程专家。请从课程资源中提取知识图谱，并严格输出 JSON，不要输出任何额外说明文字。
...
{{
  "nodes": [
    {{
      "id": "N1",
      "name": "知识点名称",
      "type": "chapter 或 knowledge_point",
      "description": "简要说明"
    }}
  ],
  "links": [
    {{
      "source": "N1",
      "target": "N2",
      "relation": "prerequisite / covers / next / related_to",
      "weight": 1.0
    }}
  ]
}}
...
要求：
1. nodes 至少 5 个，包含 chapter 与 knowledge_point 两类节点。
2. links 至少 4 条，优先使用 prerequisite / covers / next。
3. source / target 必须引用 nodes 里的 id。
4. 必须返回合法 JSON。
"""
...
class LLMClient:
    ...
    def generate_knowledge_graph(
        ...
        prompt = KNOWLEDGE_GRAPH_PROMPT_TEMPLATE.format(
            teaching_objective=(teaching_objective.strip() or "（未提供）")[:1200],
            teaching_note_text=(teaching_note_text.strip() or "（未提供）")[:12000],
            ppt_text=(ppt_text.strip() or "（未提供）")[:12000],
        )
        ...
        raw_output = self._chat(prompt)
        parsed = parse_json_with_fallback(raw_output)
        normalized = normalize_knowledge_graph_result(parsed)
        normalized["_raw_output"] = raw_output
        normalized["_prompt"] = prompt
        return normalized
...
def normalize_knowledge_graph_result(payload: dict[str, Any]) -> dict[str, Any]:
    """统一知识图谱输出结构，保证 nodes/links 可入库。"""
    ...
    for idx, item in enumerate(raw_nodes, start=1):
        if isinstance(item, str):
            ...
            node_type = str(item.get("type", "knowledge_point")).strip().lower()
        ...
        if node_type not in {"chapter", "knowledge_point"}:
            node_type = "knowledge_point"

        node_key = _normalize_graph_node_key(node_id, idx)
        key_map[node_id] = node_key
        normalized_nodes.append(
            {
                ...
                "level": 1 if node_type == "chapter" else 2,
                "order_index": idx,
            }
        )
    ...
    if len(normalized_nodes) < 2:
        ...
        key_map = {"CH_1": "CH_1", "KP_1": "KP_1"}

    node_keys = [item["node_key"] for item in normalized_nodes]
    ...
    for item in raw_links:
        ...
        source_key = key_map.get(source_raw) or _normalize_graph_node_key(source_raw, 1)
        target_key = key_map.get(target_raw) or _normalize_graph_node_key(target_raw, 1)
        if source_key not in node_keys or target_key not in node_keys or source_key == target_key:
            continue

        relation = str(item.get("relation", "related_to")).strip().lower()
        if relation not in {"prerequisite", "covers", "next", "related_to"}:
            relation = "related_to"
    ...
    if not normalized_links and len(node_keys) >= 2:
        for idx in range(len(node_keys) - 1):
            normalized_links.append(
                {
                    "source_key": node_keys[idx],
                    "target_key": node_keys[idx + 1],
...
def _normalize_graph_node_key(raw: str, index: int) -> str:
    text = re.sub(r"[^A-Za-z0-9_]", "_", raw.strip().upper())
    if not text:
        text = f"N{index}"
    return text[:60]
